...
export default function Asignar() {
        ...
        if (!isNaN(cleanedDNI)){
            try {
                ...
                if (res.data.content) {
                    ...
                    try {
                        const resp = await axios.get(`https://amddibackend-production-2880.up.railway.app/asesor/${res.data.content.carrera}`);
    ...
    async function asignarUsuarioAsesor(id_usuarios, id_asesor) {
        ...
        if (userConfirmed) {
            ...
            try {
                await axios.post(`https://amddibackend-production-2880.up.railway.app/asignaciones/${id_asesor}/${id_usuarios}`);
    ...
    async function asignarUsuarioAsesorSec(id_usuarios, id_asesor) {
        if (id_usuarios) {
            ...
            if (userConfirmed) {
                try {
                    await axios.post(`https://amddibackend-production-2880.up.railway.app/asignaciones_sec/${id_asesor}/${id_usuarios}`);
