...
export default function SubirArchivo() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // console.log(`https://amddibackend-production.up.railway.app/subir-pdf/${usuarioporDNI[0].id}`)
            const response = await axios.post(`https://amddibackend-production.up.railway.app/subir-pdf/${usuarioporDNI[0].id}`, formData);
                                ...
                                <td>
                                    <ul>
                                        {usuario.pdf_url.map((pdf, index) => (
                                            <li key={index}>
                                                <a href={`https://amddibackend-production.up.railway.app${pdf.pdf_url}`} target="_blank" rel="noopener noreferrer" download="true">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fillRule="currentColor" className="bi bi-file-type-pdf" viewBox="0 0 16 16">
